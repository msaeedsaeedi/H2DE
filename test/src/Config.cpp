...
TEST_F(ConfigTest, init) {
    EXPECT_THROW(H2DE::Config config("assets/configuration/config_init.json"), H2DE::IOException);
    EXPECT_NO_THROW(H2DE::Config config("assets/configuration/config_init.json", true));
    EXPECT_NO_THROW(H2DE::Config config("assets/configuration/config_init.json", true));
}
...
TEST_F(ConfigTest, array) {
    ...
    config.set("array.string", std::array<std::string, 3>{"Hello", "World", "!"});
    config.set("array.bool", std::array<bool, 3>{true, false, true});

    EXPECT_EQ(config.get("array.int"), (std::array<int, 3>{1, 2, 3}));
    EXPECT_EQ(config.get("array.float"), (std::array<float, 3>{1.0f, 2.0f, 3.0f}));
    EXPECT_EQ(config.get("array.string"), (std::array<std::string, 3>{"Hello", "World", "!"}));
    EXPECT_EQ(config.get("array.bool"), (std::array<bool, 3>{true, false, true}));
}
...
TEST_F(ConfigTest, array_update) {
    ...
    config.set("array.string", std::array<std::string, 3>{"World", "Hello", "!"});
    config.set("array.bool", std::array<bool, 3>{false, true, false});

    EXPECT_EQ(config.get("array.int"), (std::array<int, 3>{4, 5, 6}));
    EXPECT_EQ(config.get("array.float"), (std::array<float, 3>{4.0f, 5.0f, 6.0f}));
    EXPECT_EQ(config.get("array.string"), (std::array<std::string, 3>{"World", "Hello", "!"}));
    EXPECT_EQ(config.get("array.bool"), (std::array<bool, 3>{false, true, false}));
}
...
TEST_F(ConfigTest, object) {
    ...
    config.set("object.string", json{{"a", "Hello"}, {"b", "World"}, {"c", "!"}});
    config.set("object.bool", json{{"a", true}, {"b", false}, {"c", true}});

    EXPECT_EQ(config.get("object.int"), json({{"a", 1}, {"b", 2}, {"c", 3}}));
    EXPECT_EQ(config.get("object.float"), json({{"a", 1.0f}, {"b", 2.0f}, {"c", 3.0f}}));
    EXPECT_EQ(config.get("object.string"), json({{"a", "Hello"}, {"b", "World"}, {"c", "!"}}));
    EXPECT_EQ(config.get("object.bool"), json({{"a", true}, {"b", false}, {"c", true}}));
}
...
TEST_F(ConfigTest, object_update) {
    ...
    config->set("object.string", json{{"a", "World"}, {"b", "Hello"}, {"c", "!"}});
    config->set("object.bool", json{{"a", false}, {"b", true}, {"c", false}});

    EXPECT_EQ(config->get("object.int"), json({{"a", 4}, {"b", 5}, {"c", 6}}));
    EXPECT_EQ(config->get("object.float"), json({{"a", 4.0f}, {"b", 5.0f}, {"c", 6.0f}}));
    EXPECT_EQ(config->get("object.string"), json({{"a", "World"}, {"b", "Hello"}, {"c", "!"}}));
    EXPECT_EQ(config->get("object.bool"), json({{"a", false}, {"b", true}, {"c", false}}));
}